...
| underpaid_tolerance | float | 否 | 部分支付限额，该订单可接受的支付金额容差下限，单位为原订单币种order_currency,如果不填入，则该字段默认值为0。<br/> 当订单实收金额在上下限容差范围内时，订单状态为“已付款”; <br/> 当订单实收金额低于订单应收金额减去部分支付限额时，订单状态为“部分付款”; <br/> 当订单实收金额高于订单应收金额加上超额支付限额时，订单状态为“超额付款” <br/> 例如：原order_currency为 HKD ， underpaid_tolerance传入：3.50，则代表订单少付3.50 HKD内仍视为可接受的范围，会返回"已支付"状态，如果少付款超过3.50 HKD，则会返回"部分支付"状态 |
| overpaid_tolerance | float | 否 | 超额支付限额，该订单可接受的支付金额容差上限 ，单位为原订单币种order_currency，如果不填入，则该字段默认值为0。<br/> 当订单实收金额在上下限容差范围内时，订单状态为“已付款”; <br/> 当订单实收金额低于订单应收金额减去部分支付限额时，订单状态为“部分付款”; <br/> 当订单实收金额高于订单应收金额加上超额支付限额时，订单状态为“超额付款” <br/> 例如：原order_currency为 HKD ， overpaid_tolerance传入：3.50，则代表订单多付3.50 HKD内仍视为可接受的范围，会返回"已支付"状态，如果多付款超过3.50 HKD，则会返回"超额支付"状态 | 
| accept_partial_payment | string | 否 | 默认值：true <br/> false:  如客户不接受部分支付，需要允许用户对未完整支付的订单进行补款或退款时，则需要传入false <br/> true：如客户可接受部分支付，不需要允许用户对未完整支付的订单进行补款或退款时，则需要传入true |
| accept_over_payment | string | 否 | 默认值：true <br/> false：如客户希望允许用户对超额支付的订单进行退款时，则需要传入false <br/> true：如客户无需允许用户对超额支付的订单进行退款时，则需要传入true| 
